...
跨市场对齐使用消息刚从 `recv()` 返回时记录的 `time.monotonic()` 时间，入队后不重新打时间戳。现货没有交易所事件时间，不再借用ticker心跳或订阅额外ticker。永续另使用自身的 `E` 检查传输延迟，超过 `max_transport_lag_ms` 时独立重连；此检查要求本机系统时间准确。现货的本机接收时间不能证明其交易所侧生成时间。
...
每条bookTicker连接有独立的有界原始消息窗口（默认512条）：接收协程只记录时间并入队，不解析JSON；满时自动丢弃最旧消息。解析协程每批最多32条、约2ms后让出事件循环；已过期的原始消息在JSON解析前丢弃。解析后按交易对覆盖容量为1的最新报价槽。统计循环按 `sample_interval_ms`（当前1000ms）读取双方最新有效mid。允许缺样本，不回补；未收到新消息的旧报价不能参与计算。
...
配置文件在后台读取并热加载。数据目录、写盘队列容量修改需重启；原始消息容量变化会重建子连接，解析批量立即热加载。写盘队列满时所有新任务均不等待并计数丢弃（包括机会记录，因此落盘不保证完整）；错误和丢弃数在终端显示。每条bookTicker连接最多订阅 `book_symbols_per_connection` 个合约，独立退避重连。报价币配置变化时清理不允许的缓存。终端显示原始缓冲区占用、溢出/过期丢弃数和事件循环延迟；原始消息计数按本轮连接统计，重连后重置。丢消息是正常过载保护，并不代表保证每个交易对都有样本。`positive_basis_only=true` 仅展示和保存均值为正的交易对。
